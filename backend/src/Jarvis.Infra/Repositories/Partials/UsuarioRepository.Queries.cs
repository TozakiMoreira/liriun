...
{
    public Task<Usuario?> ObterPorId(Guid id, CancellationToken ct = default)
        => _db.Usuarios.AsNoTracking().FirstOrDefaultAsync(u => u.Id == id, ct);

    public Task<Usuario?> ObterPorEmail(string email, CancellationToken ct = default)
    {
        var normalizado = email.Trim().ToLowerInvariant();
        return _db.Usuarios.AsNoTracking().FirstOrDefaultAsync(u => u.Email == normalizado, ct);
    }

    public Task<bool> ExisteEmail(string email, CancellationToken ct = default)
    {
        var normalizado = email.Trim().ToLowerInvariant();
