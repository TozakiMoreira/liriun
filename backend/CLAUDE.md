...
## Stack
.NET 10 · ASP.NET Core Web API · EF Core 9 + Npgsql · PostgreSQL (Supabase) · JWT (HS256) + BCrypt ·
FluentValidation · Gemini API · xUnit + FluentAssertions + Moq. Solution: `Liriun.slnx`.
...
- **Gemini atrás de `IGeminiService`** — Application não conhece HTTP nem Google. Config via Options tipado. Tratar 429 (retry hint) e JSON inválido (fallback) explicitamente.
...
- Contrato mudou? Regenerar clients (Dart + TS via OpenAPI) faz parte do mesmo PR — o backend é dono do OpenAPI spec.
