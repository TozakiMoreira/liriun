...
import { tarefasApi } from "@/lib/api/tarefas";
import type { CriarTarefaPayload, Tarefa } from "@/lib/api/types";

const KEY_PEND = ["tarefas", "pendentes"] as const;
const KEY_CONC = ["tarefas", "concluidas"] as const;

export function useTarefasPendentes() {
  return useQuery({ queryKey: KEY_PEND, queryFn: tarefasApi.listarPendentes });
}
